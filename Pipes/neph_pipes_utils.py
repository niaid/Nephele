...
class Config:
    ...
    VALID_ANALYSIS_TYPE = ['DE_NOVO', 'OPEN_REFERENCE', 'CLOSED_REFERENCE', 'OPEN_REFERENCE_ITS']
    
    # VALID_BS_LIST = ["NONE", "ALL", "ATTACHED_KERATINIZED_GINGIVA", "BUCCAL_MUCOSA", "HARD_PALATE", "TONGUE_DORSUM", "PALATINE_TONSILS", "SUBGINGIVAL_PLAQUE", "SUPRAGINGIVAL_PLAQUE", "SALIVA", "THROAT", "ANTERIOR_NARES", "LEFT_ANTECUBITAL_FOSSA", "RIGHT_ANTECUBITAL_FOSSA", "LEFT_RETROAURICULAR_CREASE", "RIGHT_RETROAURICULAR_CREASE", "STOOL", "VAGINAL_INTROITUS", "MID_VAGINA", "POSTERIOR_FORNIX"]
            ...
            if value == '':
                ...
                if value.upper() in self.VALID_INPUT_TYPE:
                    ...
                    raise ValueError('{0} can only be one of the following:{1}. "{2}" not permitted.'\
                                     .format(key, ','.join( self.VALID_INPUT_TYPE), value))
...
#                         raise ValueError('{0} can only be one of the following:{1}. "{2}" not permitted.'.format(key, v))
    ...
    def link_seqs_fasta( self ):
        return [ File_to_link_name( fname = cfg.SPLIT_LIB_OUT_DIR+'/seqs.fna',
                                    lname = cfg.COLLATED_OUT_DIR+'/reads_used_for_analysis.fasta' )]
    
    def link_config_run_params( self ):
        return [ File_to_link_name( fname = 'config.csv',
                                   lname = cfg.COLLATED_OUT_DIR + '/job_parameters_run_settings.csv')]
    ...
    def link_samples_being_ignored( self ):
        return [ File_to_link_name( fname =cfg.PICK_OTUS_OUT_DIR + '/samples_being_ignored.txt',
                                    lname = cfg.COLLATED_OUT_DIR) ]
    ...
    def is_OK_SE_fq_mapping_file( fname ):
    ...
    def load_SE_fq_mapping_file( fname ):        
        with open(PSN_file) as f:
    ...
    def load_map_file( self ):
        ...
        Sample_SE_mplex = namedtuple('Sample_SE_mplex',
                                     ['sample_id',
                                      ...
                                      'Description'] )
        l = list()
        # if self.pipe_name == 'FASTQ_SINGLE-END' and is_OK_SE_fq_mapping_file( self.map_file ):
        #     if not self.is_demultiplex:
        #     l  = load_SE_fq_mapping_file( self.map_file )
        ...
        with open ( file_exists(self.map_file), 'r') as f:
            for line in nonblank_lines(f):
                ...
                if self.pipe_name == 'MISEQ_MULTIPLEX':
                    line_as_list = line.split("\t")
                    # 0 SampleID  # 1 BarcodeSequence  # 2 LinkerPrimerSequence # 3 ReversePrimer
                    # 4 ForwardFastqFile # 5 ReverseFastqFile # 6 TreatmentGroup # 7 Description
                    treatmentgroup = ','.join(line_as_list[6:-1])

                    s = Sample(sample_id = line_as_list[ 0 ],\
                               BarcodeSequence = line_as_list[ 1 ],\
                               LinkerPrimerSequence = line_as_list[ 2 ],\
                               fwd_fq_file = file_exists( line_as_list[ 4 ] ),\
                               rev_fq_file = file_exists( line_as_list[ 5 ] ),\
            ...
            if self.pipe_name == 'MISEQ_PAIR-END':
                for sample in self.samples:
                    if not os.path.isfile( sample.fwd_fq_file ):
                        log_no_run_bc_missing_file( sample.fwd_fq_file, 'join_pair_ends.py' )
                ...
                if not os.path.isfile( self.fwd_fq_file ):
                    log_no_run_bc_missing_file( self.fwd_fq_file, 'join_pair_ends.py' )
        ...
        if self.pipe_name == 'MISEQ_PAIR-END':
            for sample in self.samples:
                if not os.path.isfile( sample.sample_id + '/' + 'map.txt' ):
                    log_no_run_bc_missing_file( sample.sample_id + '/' + 'map.txt',
                                                'validate_mapping_file.py' )
                else:
                    self.cmds.append('validate_mapping_file.py '\
                                     + ' --mapping_fp=' + sample.sample_id + '/' + 'map.txt'\
                                     + ' --output_dir=' + sample.sample_id )
        else:
            if not os.path.isfile( self.map_file ):
                log_no_run_bc_missing_file ( self.map_file, 'validate_mapping_file.py')
            else:
    ...
    def gen_convert_fastaqual_fastq_cmd( self ):
        if self.pipe_name == 'FASTQ_SINGLE-END' and not self.is_demultiplex:
            if not os.path.isfile( self.fastq_file ):
                log_no_run_bc_missing_file( self.fastq_file, 'convert_fastaqual_fastq.py' )
            else:
...
#                               'class=TreatmentGroup'] )
    ...
    def make_mothur_chimera_cmd( self ):
        if self.chimera:
    ...
    def gen_make_otu_table_cmd( self ):
        if self.chimera and self.lookup_accnos_file() \
           and os.path.isfile(
               cfg.PICK_OTUS_OUT_DIR + '/uclust_assigned_taxonomy/seqs_rep_set_tax_assignments.txt' )\
               and self.analysis_type != 'CLOSED_REFERENCE':
            ret = 'make_otu_table.py '\
                + ' --exclude_otus_fp=' + self.lookup_accnos_file() \
                + ' --output_biom_fp=' + cfg.PICK_OTUS_OUT_DIR + '/otu_table_clean.biom'\
                + ' --otu_map_fp=' + cfg.PICK_OTUS_OUT_DIR + '/uclust_assigned_taxonomy/seqs_rep_set_tax_assignments.txt'
        
            if self.analysis_type == 'OPEN_REFERENCE':
                ret += ' --taxonomy=' + cfg.PICK_OTUS_OUT_DIR + \
